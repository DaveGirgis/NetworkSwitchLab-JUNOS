...
ge-0/0/1         128:2        128:2   8192.000586714cd0     20000  FWD    DESG
ge-0/0/2         128:3        128:3   8192.000586714cd0     20000  FWD    DESG
ge-0/0/3         128:4        128:4   4096.0005867107d0     20000  BLK    ALT
```

`ge-0/0/0` is the **Root port** — SW2's best path to the root bridge (Designated bridge ID shows SW1).
`ge-0/0/3` is the **Alternate port** — blocked. Note the Designated bridge ID for ge-0/0/3 is also SW1's (`4096...`), not SW2's. This is correct: SW1's ge-0/0/3 is the Designated port on that segment, so SW1's bridge ID is advertised there. SW2 loses the election on that segment and puts its port into Alternate.
`ge-0/0/1` and `ge-0/0/2` show SW2's own bridge ID — SW2 is the sole switch on those access segments and is therefore Designated.

!!! note "Which trunk gets blocked?"
    Both trunks have the same port cost (20000), so the tie is broken by port ID. The higher port number (ge-0/0/3) on SW2 loses and becomes the Alternate. MAC addresses in your output will differ but the ROOT/ALT roles should match.
